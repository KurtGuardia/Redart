...
export default function AddSpace() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!auth.currentUser) {
      setError("Debes iniciar sesión para agregar un espacio")
      return
    }
    ...
    try {
      let imageUrl = null
      if (image) {
        const imageRef = ref(storage, `spaces/${auth.currentUser.uid}/${Date.now()}-${image.name}`)
  ...
  return (
    <Layout>
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-8">Agregar nuevo espacio</h1>
